...
//        int stopCount = 0;
//        int rowLimit = paper.getPaperRow() - printer.getPrinterRow() + 1;
//        int colLimit = paper.getPaperCol() - printer.getPrinterCol() + 1;
//
//        // 슬라이딩 윈도우를 오른쪽-아래 방향으로 이동하며 검사
...
//    }
//
//    private boolean checkMatch(int startX, int startY) {
//        // 프린터가 해당 위치에서 정확히 일치하는지 검사
//        for (int i = 0; i < printer.getPrinterRow(); i++) {
//            for (int j = 0; j < printer.getPrinterCol(); j++) {
//                if (printer.getCoordinate(i, j) != paper.getCoordinate(startX + i, startY + j)) {
//                    return false; // 일치하지 않는 경우 조기 종료
//                }
//            }
//        }
//        return true;
